...
private fun defaultCategories(user: FirebaseUser){
    val db = Firebase.firestore
    val categorias = listOf(
        hashMapOf<String, String>("nome" to "Sites web", "quantidade" to "0"),
        hashMapOf<String, String>("nome" to "Aplicativos", "quantidade" to "0"),
        hashMapOf<String, String>("nome" to "Teclado de acesso físico", "quantidade" to "0")
    )
    val batch = db.batch()

    for(categoria in categorias){
        val docRef = db.collection("users").document(user.uid)
            .collection("categorias").document(categoria["nome"].toString())
        batch.set(docRef,categoria)
    }
    batch.commit()
        .addOnSuccessListener {
            println("Categorias padrão criadas com sucesso!")
        }
        .addOnFailureListener { e ->
